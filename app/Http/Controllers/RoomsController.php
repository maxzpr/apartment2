...
class RoomsController extends Controller
{
    ...
    {
        $rooms = RoomModel::select('rooms.id as room_id','shname','rent','cost','roomer','type','is_empty')
    ...
    {
        $this->validate($request,['room_id'=>'required','roomer'=>'required','rent'=>'required']);
        $ap_name = ApartModel::find($request->get('ap'))->toArray()['shname']; 
        $room_id = $ap_name.'_'.$request->get('room_id');
        $cost = !empty($request['cost'])?$request['cost']:[];
        $room = new RoomModel([
            'id'     => $room_id,
    ...
    public function update(Request $request, $id)
    {
        ...
        $room->is_empty= !empty($request['is_empty'])?1:0;
